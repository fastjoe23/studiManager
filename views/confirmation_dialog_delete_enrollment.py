...
class ConfirmationDialogDeleteEnrollment(tk.Toplevel):
    def __init__(self, parent, enrollment, callback, *args, **kwargs):
        ...
        self.label = tk.Label(self, text="Möchten Sie den Studenten:")
        ...
        course = self.master.master.controller.read_course_by_id(enrollment.course_id)
        # zugehoerigen Studenten lesen
        student = self.master.master.controller.read_student_by_id(enrollment.student_id)
        ...
        self.label = tk.Label(self, text=f"wirklich aus dem Kurs {course.course_name} löschen?")
        self.label.pack(pady=10)
