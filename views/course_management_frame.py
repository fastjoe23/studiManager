...
class CourseManagementFrame(tk.Frame):
    def __init__(self, parent, course_id, *args, **kwargs):
        ...
        add_student_button = tk.Button(
            actions_frame,
            text="Student zu Kurs hinzufügen",
            command=lambda: self.add_student_to_course(parent, course_id),
            width=button_width,
        )
        ...
        send_mail_button = tk.Button(
            actions_frame,
            text="Kurs Mail versenden",
            command=lambda: self.send_course_mail(course_id),
            width=button_width,
        )
        send_mail_button.pack(pady=5)

        create_evaluation_pdfs_button = tk.Button(
            actions_frame,
            text="Begutachtungsformulare erstellen",
            command=lambda: self.create_evaluation_pdfs(course_id),
            width=button_width,
        )
        ...
        import_list_button = tk.Button(
            actions_frame,
            text="Kurs aus Liste einlesen",
            command=lambda: self.import_course_from_excel_list(course_id),
            width=button_width,
        )
        import_list_button.pack(pady=5)

        import_assignment_list_button = tk.Button(
            actions_frame,
            text="Arbeiten aus Liste einlesen",
            command=self.import_assignments_from_list,
            width=button_width,
        )
        import_assignment_list_button.pack(pady=5)

        generate_list_button = tk.Button(
            actions_frame,
            text="Kursliste aus Mail generieren",
            command=lambda: self.generate_course_list(course_id),
            width=button_width,
        )
        generate_list_button.pack(pady=5)

        generate_assignment_list_button = tk.Button(
            actions_frame,
            text="Blanko Arbeiten Liste generieren",
            command=lambda: self.generate_assignment_list(course_id),
            width=button_width,
        )
        ...
        close_course_button = tk.Button(
            actions_frame,
            text="Kurs verlassen",
            command=lambda: self.close_course_management(course_id),
            width=button_width,
        )
        ...
        students_label = tk.Label(
            students_frame,
            text="Eingeschriebene Studenten:",
        ...
        yscrollbar_notes = ttk.Scrollbar(
            students_frame, orient="vertical", command=self.tree.yview
        )
        self.tree.configure(yscrollcommand=yscrollbar_notes.set)

        yscrollbar_notes.pack(side=tk.RIGHT, fill=tk.Y)
        # Tag fuer inaktive Studenten konfigurieren
        ...
        self.tree.bind(
            "<Double-1>",
            lambda event, course_id=course_id: self.on_student_double_click(
                event, course_id
            ),
        )
        ...
        self.add_note_button = tk.Button(
            notes_label_frame, text="+", command=lambda: self.add_note(course_id)
        )
        self.add_note_button.pack(fill=tk.X, side=tk.RIGHT, padx=10, pady=5)

        # Treeview für Notizen
        self.note_treeview = ttk.Treeview(
            notes_frame, columns=("note_id", "title", "kurzfassung"), show="headings"
        )
        ...
        # Vertikale Scrollbar hinzufügen
        yscrollbar_notes = ttk.Scrollbar(
            notes_frame, orient="vertical", command=self.note_treeview.yview
        )
        ...
        # Doppelklick-Ereignis auf den Treeview binden
        self.note_treeview.bind(
            "<Double-1>",
            lambda event, course_id=course_id: self.on_note_double_click(
                event, course_id
            ),
        )
    ...
    def import_course_from_csv_list(self, course_id):
        ...
        if csv_path:
            try:
                ...
                self.master.controller.import_students_from_csv_into_course(
                    csv_path, course_id
                )
    ...
    def import_course_from_excel_list(self, course_id):
        ...
        excel_path = filedialog.askopenfilename(
            defaultextension=".xlxs", filetypes=[("Excel Dateien", "*.xlsx")]
        )
        ...
        if excel_path:
            try:
                # Rufe die Methode zum Importieren von Studenten in den Kurs auf
                self.master.controller.import_students_from_excel_into_course(
                    excel_path, course_id
                )
            except Exception as e:
                # Zeige eine Messagebox mit dem Inhalt der Exception an
                messagebox.showerror("Fehler beim Importieren", str(e))
    ...
    def import_assignments_from_list(self):
        # Öffne einen Dateidialog, um die CSV-Datei auszuwählen
        excel_path = filedialog.askopenfilename(
            defaultextension=".xlsx", filetypes=[("Excel Dateien", "*.xlsx")]
        )
        ...
        if excel_path:
            try:
                ...
                imported_assignments = (
                    self.master.controller.import_assignments_from_excel_into_course(
                        excel_path
                    )
                )
                messagebox.showinfo(
                    "Import",
                    f"Es wurden {len(imported_assignments)} Arbeiten erfolgreich importiert.",
                )
    ...
    def load_enrolled_students(self, course_id):
        ...
        for student in enrolled_students:
            student_enrolled_tag = () if student.enrolled else ("inactive",)
    ...
    def load_course_notes(self, course_id):
        ...
        for note in list_of_notes:
            self.note_treeview.insert(
                "", tk.END, values=(note.note_id, note.note_title, note.note[:50])
            )
